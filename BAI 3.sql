...
BEGIN
    ...
    ELSE
        CASE p_patient_type
            WHEN 'BHYT' THEN 
                SET p_final_amount = p_total_cost * 0.2; -- 80%, đóng 20%
            WHEN 'VIP' THEN 
                SET p_final_amount = p_total_cost * 0.9; -- 10%, đóng 90%
            WHEN 'THUONG' THEN 
...
-- Phải truyền đủ 2 biến @ cho 2 tham số OUT
CALL CalculateHospitalFee(-1000000, 'BHYT', @output_money, @output_msg);

-- Xem kết quả
SELECT @output_money AS 'Số tiền thực thu', @output_msg AS 'Thông báo từ hệ thống';